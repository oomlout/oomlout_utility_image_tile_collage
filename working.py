...
def main(**kwargs):
    ...
    image_source = kwargs.get('image_source', 'image.jpg')
    ...
    if file_input != '':        
        ...
        file_image_source = f'{directory_single}/{image_source}'
        job = copy.deepcopy(job_default)
        job['file_image_source'] = file_image_source
        ...
        for directory in directories:
            if os.path.isdir(f'{directory_iterate}/{directory}'):
                file_image_source = f'{directory_iterate}/{directory}/{image_source}'
                #find the image file if no extension try common extensions
                if True:
                    if not os.path.exists(file_image_source):
                        extensions = ['.png', '.jpg', '.jpeg', '.bmp', '.tif', '.tiff']
                        found = False
                        for ext in extensions:
                            file_image_source_try = f'{directory_iterate}/{directory}/{image_source}{ext}'
                            if os.path.exists(file_image_source_try):
                                file_image_source = copy.deepcopy(file_image_source_try)
                                found = True                                
                                break
                        if not found:
                            print(f'file not found: {file_image_source}')
                            continue



                job = copy.deepcopy(job_default)
                job['image_source'] = file_image_source
...
def run_job(**kwargs):
    ...
    if not os.path.exists(overwrite_test_direcrtory) or overwrite:
        ...
        print(f'skipping job for output directory (exists): {output_directory}')
...
if __name__ == '__main__':
    ...
    # stretch to fit postcards default false
    argparser.add_argument('--stretch_to_fit_postcards', '-stfp', type=bool, default=False, help='stretch_to_fit_postcards')
    # image_source -is
    argparser.add_argument('--image_source', '-is', type=str, default='image.jpg', help='image_source, omit extension if it varies common ones will be tested')
    # overwrite existing -oe default False
    argparser.add_argument('--overwrite_existing', '-oe', type=bool, default=False, help='overwrite_existing')
